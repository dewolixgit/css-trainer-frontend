...
export const task9InputItemsExtractor: InputItemsExtractor = (inputs) => {
...
export class Task9StylistPlugin extends BaseTaskStylistPlugin implements ITaskStylistPlugin {
  stylize(): string {
    const value = task9InputItemsExtractor(this._inputs)[0].value;
...
export class Task9CheckerPlugin extends BaseTaskCheckerPlugin implements ITaskCheckerPlugin {
  check(): boolean {
    const value = task9InputItemsExtractor(this._inputs)[0].value;

    return value === `.${SET_SELECTORS_ELEMENTS_ATTRIBUTES.spell.className}`;
  }
}
