...
export const task8InputItemsExtractor: InputItemsExtractor = (inputs) => {
...
export class Task8StylistPlugin extends BaseTaskStylistPlugin implements ITaskStylistPlugin {
  stylize(): string {
    const value = task8InputItemsExtractor(this._inputs)[0].value;
...
export class Task8CheckerPlugin extends BaseTaskCheckerPlugin implements ITaskCheckerPlugin {
  check(): boolean {
    const value = task8InputItemsExtractor(this._inputs)[0].value;

    return value === `.${SET_SELECTORS_ELEMENTS_ATTRIBUTES.helmet.className}`;
  }
}
