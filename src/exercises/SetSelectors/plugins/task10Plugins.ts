...
export const task10InputItemsExtractor: InputItemsExtractor = (inputs) => {
...
export class Task10StylistPlugin extends BaseTaskStylistPlugin implements ITaskStylistPlugin {
  stylize(): string {
    const value = task10InputItemsExtractor(this._inputs)[0].value;
    ...
    const prepared = (() => {
      if (isTag) {
        return `svg[data-service-tag=${sanitized}]`;
      }

      if (checkIsValid(sanitized)) {
        return `#${SET_SELECTORS_ELEMENTS_ATTRIBUTES.sword.id}`;
      }
...
export class Task10CheckerPlugin extends BaseTaskCheckerPlugin implements ITaskCheckerPlugin {
  check(): boolean {
    const value = task10InputItemsExtractor(this._inputs)[0].value;
